...
def postprocess_qa_predictions(
    ...
    # example과 mapping되는 feature 생성, k = 'mrc-xxx', i = 0~num
    ...
    for example_index, example in enumerate(tqdm(examples)):
        ...
        for feature_index in feature_indices:
            ...
            norm_end_logits = np.array(end_logits).reshape(-1, 1)
            norm_end_logits = scaler.fit_transform(norm_end_logits).flatten().tolist()  # start_logits 정규화
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    ...
                    for index in range(len(example['doc_offsets'])):
                        if index < len(example['doc_offsets'])-1:
                            if example['doc_offsets'][index] <= start < example['doc_offsets'][index+1]:
                                doc_num = index
                        elif index == len(example['doc_offsets'])-1:
